...
def delete_news(message):
 ...
 for n in news: # ВЫВОД ВСЕХ НОВОСТЕЙ С ИХ ИНДЕКСОМ новости
    bot.send_message(message.chat.id, f'<b>id {n[0]}-></b>   {n[1]}'); print('ready to del news', n) 
...
def make_food_menu(message):
 ...
 meals_numbers_for_free_breakfast = bot.send_message(message.chat.id, 'Введите номера блюд для бюджетного завтрака через запятую')
...
def show_wall(message):
 ...
 wall_msgs=sql.execute('SELECT date, user_id, user_msg FROM wall ORDER BY id DESC LIMIT 6').fetchall() # ВЫВОДИМ 6 последних СООБЩЕНИЙ
...
def lalala_main_text_message_handler(message):
 if message.text=='Уведомление начала и конца уроков': bot.send_message(message.chat.id,"Сейчас на сервере " + str(datetime.now())+'Эту функцию еще не разработали')
 elif message.text=='Личный кабинет':  personal_cabinet(message)
 elif message.text=='Викторина (QUIZ)':  quiz(message) 
 elif message.text=='Вопрос': question(message)
