...
sql.execute("INSERT INTO `testTable` VALUES (NULL,'helo',1494,1)")            # Добавляем запписи
sql.execute("INSERT INTO `testTable` (`t_pole`, `numb`) VALUES ('Русский',1223)") # разными способами
...
print (' kolichestvo zapisey (РУС)' , end=' ') ; print(   num_   );

sql.execute('UPDATE `testTable` SET t_pole="fff" where id>=2 AND id<4 ') # Изменяем запись
sql.execute("UPDATE `testTable` SET `t_pole` = ? WHERE `id` = ?", ( "NEW! Русский" , 1) )
db.commit()
...
if bool(len(result))==False: # Если записи с таким айди нет , то добавляем ее
 sql.execute("INSERT INTO `testTable` (`t_pole`) VALUE(?) ", ('Dobavka if not exists') )
else:
 sql.execute("UPDATE `testTable` SET  `t_pole`= ? WHERE id=?", ( 'Obnovka',3))
